...
def train_epoch(selector, selector_name, models_list, student, student_name, 
    ...
    for x, y_true, metadata in train_loader:
        ...
        #t_1 = time.time()
        with open('log/fmow/'+curr+'.txt', 'a+') as f:
        ...
        if i == iter_per_epoch//2:
            ...
            worst_acc, acc = eval(selector, models_list, student, sup_size, device=device,
                        ilr=ilr, test=False, progress=False, uniform_over_groups=False,
                        root_dir=root_dir)
            
            with open('log/fmow/'+curr+'.txt', 'a+') as f:
                f.write(f'Accuracy: {acc}; Worst acc: {worst_acc}\r\n')
            losses = []
...
def train_kd(selector, selector_name, models_list, student, student_name, split_to_cluster, device,
    ...
    train_loader, _, _, grouper = get_data_loader(root_dir=root_dir, batch_size=batch_size, domain=None,
                                                  ...
                                                  groupby_fields=['year', 'region'])

    mask_grouper = get_mask_grouper(root_dir=root_dir)

    curr = str(datetime.now())
    if not os.path.exists("log/fmow"):
        os.makedirs("log/fmow")
    #print(curr)
    print("Training log saved to log/fmow/"+curr+".txt")
    ...
    for epoch in range(num_epochs):
        ...
        worst_acc, accu = eval(selector, models_list, student, sup_size, device=device, 
                    ilr=ilr, test=False, progress=False, uniform_over_groups=False,
                    root_dir=root_dir)

        with open('log/fmow/'+curr+'.txt', 'a+') as f:
            f.write(f'Accuracy: {accu}; Worst acc: {worst_acc}\r\n')
        
        if worst_acc > accu_best and save:
            with open('log/fmow/'+curr+'.txt', 'a+') as f:
                f.write("Saving model ...\r\n")
            save_model(selector, selector_name+"_selector", 0, test_way=test_way)
            save_model(student, student_name+"_student", 0, test_way=test_way)
...
def eval(selector, models_list, student, batch_size, device, ilr=1e-5,
    ...
    if test:
        loader, grouper, dataset = get_test_loader(batch_size=batch_size, split='test', root_dir=root_dir,
                                                groupby_fields=['year', 'region'], return_dataset=True)
    else:
        loader, grouper, dataset = get_test_loader(batch_size=batch_size, split='val', root_dir=root_dir,
                                                groupby_fields=['year', 'region'], return_dataset=True)
