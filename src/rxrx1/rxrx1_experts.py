...
def train_model(model, model_name, device, domain=None, batch_size=75, lr=1e-4, l2=1e-5, 
                num_epochs=90, decayRate=1., save=False, test_way='ood', root_dir='data'):
    ...
    scheduler = get_cosine_schedule_with_warmup(
                optimizer,
                num_warmup_steps=tot * 10,
                num_training_steps=tot*num_epochs)
...
def train_exp(models_list, domain_specific_indices, device, batch_size=75, lr=1e-4, l2=1e-5,
          num_epochs=90, decayRate=1., save=False, test_way='ood', name="Resnet50_experts",
          root_dir='data'):
